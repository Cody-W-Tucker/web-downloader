...
class ContentExtractor:
    ...
    def _extract_author(self, soup):
        ...
        author_elements = soup.find_all(
            ["a", "span", "div"], class_=re.compile(r"author|byline", re.I)
        )
    ...
    def _is_content_element(self, element):
        ...
        for attr in ["class", "id"]:
            ...
            if attr_value and any(
                pattern in attr_value.lower()
                for pattern in [
                    "nav",
                    "menu",
                    "sidebar",
    ...
    def extract_main_content(self, soup):
        ...
        try:
            ...
            for selector in [
                ...
                if (
                    content
                    and len(content.get_text(strip=True)) > self.min_content_length
                ):
                    return content
    ...
    def clean_content(self, content):
        ...
        try:
            ...
            for element in new_content.find_all(
            ...
            for element in new_content.find_all(
                class_=re.compile(
                    r"share|social|comment|ad|sidebar|nav|menu|popup|cookie"
                )
            ):
                element.decompose()

            for element in new_content.find_all(
                id=re.compile(r"share|social|comment|ad|sidebar|nav|menu|popup|cookie")
            ):
                element.decompose()
    ...
    def extract_content(self, html_content, url):
        ...
        try:
            ...
            content_length = (
                len(cleaned_content.get_text(strip=True)) if cleaned_content else 0
            )
