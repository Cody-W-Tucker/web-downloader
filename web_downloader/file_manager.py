...
class FileManager:
    ...
    def __init__(self, output_dir="output"):
        ...
        self.output_is_domain_based = (
            output_dir.startswith("./")
            and "/" not in output_dir[2:]
            and "\\" not in output_dir[2:]
        )
    ...
    def url_to_filepath(self, url):
        ...
        directory = (
            os.path.join(self.output_dir, *dir_components)
            if dir_components
    ...
    def save_markdown(self, markdown_content, url, title=None):
        ...
        try:
            ...
            content_with_frontmatter = self._add_frontmatter(
                markdown_content, url, title
            )

            # Handle naming conflicts
            filepath = self._handle_naming_conflict(
                filepath, url, content_with_frontmatter
            )
    ...
    def save_youtube_transcript(
        self, video_info: dict, markdown_content: str
    ) -> str | None:
        """
        ...
        try:
            ...
            logger.error(
                f"Error saving YouTube transcript for {video_id or 'unknown'}: {str(e)}"
            )
            return None
    ...
    def _handle_naming_conflict(self, filepath, url, content_with_frontmatter):
        ...
        try:
            ...
            source_url = (
                (
                    existing_frontmatter.get("source_url")
                    or existing_frontmatter.get("youtube_url")
                )
        ...
        while True:
            ...
            if not os.path.exists(new_filepath):
                logger.debug(
                    f"Renamed {filepath} to {new_filepath} to avoid conflict with different content"
                )
                return new_filepath
            ...
            try:
                ...
                source_url = (
                    (
                        numbered_frontmatter.get("source_url")
                        or numbered_frontmatter.get("youtube_url")
                    )
                    if numbered_frontmatter
                    else None
                )
                if source_url and self._normalize_url(source_url) == normalized_new_url:
                    # Found a match with an existing numbered version, update it
                    logger.debug(
                        f"Updating existing numbered file {new_filepath} from same URL"
                    )
                    return new_filepath
            except Exception:
                pass
