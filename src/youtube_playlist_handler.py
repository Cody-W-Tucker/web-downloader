...
class YouTubePlaylistHandler:
    ...
    def get_videos_from_playlist(self, playlist_id):
        ...
        """
        videos = []
        cache_file = f".cache/videos_{playlist_id}.json"
        if os.path.exists(cache_file):
            self.logger.info(f"Loading cached video list from {cache_file}")
            try:
                with open(cache_file, "r", encoding="utf-8") as f:
                    videos = json.load(f)
                self.logger.info(f"Loaded {len(videos)} videos from cache")
            except Exception as e:
                self.logger.warning(f"Invalid cache {cache_file}, fetching fresh: {e}")
        ...
        try:
            ...
            for err in errors:
                ...
                if reason in ["tooManyRequests", "internalError", "serviceUnavailable"]:
                    ...
                    self.logger.warning(
                        "YouTube API quota exceeded. Please wait 24h or use a different API key."
                    )
                    return False
        except Exception:
            pass
    ...
    def _execute_with_retry(self, request, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                self._log_api_error(
                    e, f"API request failed (attempt {attempt + 1}/{max_retries})"
                )
