            ...
            try:
                ...
                try:
                    with urllib.request.urlopen(request) as response:
                        ...
                        if crawl_delay:
                            logger.info(
                                f"Found crawl delay of {crawl_delay}s for {domain}"
                            )
                    ...
                    logger.warning(
                        f"HTTP error {e.code} fetching robots.txt from {domain}: {str(e)}"
                    )
                    logger.debug(
                        f"Response headers: {dict(e.headers) if hasattr(e, 'headers') else 'No headers'}"
                    )
                    ...
                    if e.code == 403:
                        ...
                        logger.warning(
                            f"Access to robots.txt forbidden (403) for {domain}. Using {self.default_403_policy} policy."
                        )
                ...
                try:
                    ...
                    logger.warning(
                        f"Error reading robots.txt with standard method from {domain}: {str(e)}"
                    )
                ...
                logger.warning(
                    f"Unexpected error handling robots.txt for {domain}: {str(general_e)}"
                )
    ...
    def can_fetch(self, url):
        ...
        try:
            ...
            if domain in self.denied_robots:
                ...
                if self.default_403_policy == "allow":
                    logger.debug(
                        f"Access allowed by {self.default_403_policy} policy for {url}"
                    )
                    return True
                ...
                if (
                    path
                    in ["/", "/index.html", "/index.php", "/sitemap.xml", "/robots.txt"]
                    or path == ""
                ):
                    logger.debug(
                        f"Access to common path allowed despite 403 robots.txt: {url}"
                    )
                    return True

                # Block for non-allowed paths in conservative mode
                logger.warning(
                    f"Access denied by {self.default_403_policy} policy for {url}"
                )
                return False
            ...
            if hasattr(parser, "sitemaps") and parser.sitemaps:
                for sitemap in parser.sitemaps:
                    if url == sitemap:
                        logger.info(
                            f"Explicitly allowing sitemap from robots.txt: {url}"
                        )
                        return True
            ...
            if result:
                ...
                if any(path.endswith(sitemap) for sitemap in common_sitemaps):
                    logger.warning(
                        f"Allowing common sitemap URL despite robots.txt: {url}"
                    )
                    return True
                ...
                if path == "/" or path == "":
                    logger.warning(
                        f"Root path disallowed by robots.txt parser, but this may be a parser error: {url}"
                    )
                    return True
